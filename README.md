...
| Sim14 | 原油 +50%（恒久）。為替はモデル内で反応 | 論文になし |
| Sim15 | 原油 +50% ＋ 20% 円安（両方を固定） | 論文になし |
| Sim16 | Sim15 に加えてコールレートを8四半期据え置き | 論文になし |
...
2021Q4〜2022Q3 の動き（Brent が約 +50%、ドル円 114 → 144）を目安にした複合供給ショックです。Sim14 は原油だけを動かして為替をモデルに任せ、Sim15 は円安も外から固定し、Sim16 はさらにコールレートを8四半期ベースラインに据え置きます。

| | 1期 | 4期 | 8期 | 12期 | 20期 |
|---|---|---|---|---|---|
| **Sim14** 実質GDP [%] | −0.02 | −0.42 | −0.75 | −0.77 | −0.29 |
| **Sim14** コアCPI [%] | +0.24 | +0.54 | +0.43 | +0.22 | −0.37 |
| **Sim15** 実質GDP [%] | +0.42 | +0.13 | −0.21 | −0.36 | −0.01 |
| **Sim15** コアCPI [%] | +0.36 | +0.88 | +0.92 | +0.89 | +0.72 |
| **Sim16** 実質GDP [%] | +0.42 | +0.17 | −0.13 | −0.28 | +0.03 |
| **Sim16** コアCPI [%] | +0.36 | +0.88 | +0.93 | +0.93 | +0.83 |

Sim14 は教科書どおりのスタグフレーションで、実質GDPが8〜12期に −0.75% まで落ち、コアCPIは4期に +0.54% まで上がります。ただし**物価の押し上げは続きません**。需給ギャップの悪化で税抜きの国内物価が下がっていくため、20期にはコアCPIが −0.37% とマイナスに転じます。政策金利も同じ形で、4期に +0.23%ポイントまで上げたあと、20期には −0.53%ポイントまで下げます。

円安を加えると（Sim15）、実質輸出が +1.9〜2.6% 増えてGDPの落ち込みが半分以下（8期で −0.75% → −0.21%）になる一方、コアCPIは高く、長く上がり続けます（20期で −0.37% → +0.72%）。**円安はスタグフレーションの「スタグ」を和らげて「フレーション」を強める**という整理になります。実質消費は逆に悪化し（20期で −1.38% → −2.33%）、輸入が −6.6% まで落ち込みます。金利を据え置くと（Sim16）この傾向がもう一段進み、GDPはわずかに改善、コアCPIは20期で +0.83% と高止まりします。

なお大きなショックに対する反応は線形ではありません。Sim2（原油 −10%）を −5倍して外挿すると8期の実質GDPは −0.94% ですが、実際に +50% を与えた Sim14 では −0.75% と 2割ほど小さくなります。

![Sim14 vs Sim15](output/Sim14_vs_Sim15.png)
